...
namespace Zeus.Framework.Asset
{
    ...
    public static class AssetBundleUtils
    {
        ...
        {
            var unloadbleBundles = new HashSet<string>();
            //parse alone asset bundle
            Dictionary<string, int> bundles2AssetCount = new Dictionary<string, int>();
            foreach (var pair in _assetMapBundles)
            {
                if (bundles2AssetCount.ContainsKey(pair.Value))
                {
                    bundles2AssetCount[pair.Value]++;
                }
                else
                {
                    bundles2AssetCount.Add(pair.Value, 1);
                }
            }
            //parse not be depended bundle
            var bundles = _manifest.GetAllAssetBundles();
            HashSet<string> beDepBundles = new HashSet<string>();
            foreach (var bundle in bundles)
            {
                var deps = _manifest.GetAllDependencies(bundle);
                if (deps != null)
                {
                    foreach (var dep in deps)
                    {
                        beDepBundles.Add(dep);
                    }
                }
            }
            //filter unloadble bundle
            Stream stream = null;
            try
            {
                stream = VFileSystem.OpenFile(_GetRelativeBundlePath(AssetMapNameXml), FileMode.Open, FileAccess.Read);
                XmlDocument doc = new XmlDocument();
                doc.Load(stream);
                XmlNode rootNode = doc.SelectSingleNode("Assets");
                foreach (XmlNode assetNode in rootNode.ChildNodes)
                {
                    ...
                    string assetPath = assetElement.GetAttribute("path");
                    XmlNode bundleNode = assetElement.FirstChild;
                    string bundleName = bundleNode.InnerText;
                    if (bundles2AssetCount[bundleName] > 1 || beDepBundles.Contains(bundleName))
                    {
                        continue;
                    }
                    string assetExt = assetElement.GetAttribute("ext");
                    if(assetExt == ".prefab" || assetExt == ".unity")
                    {
                        continue;
                    }
                    unloadbleBundles.Add(bundleName);
                }
            }
            catch (System.Exception e)
            {
        ...
        public static string[] GetAllDependencies(string abName)
        {
            ...
            int i = 0;
            foreach (string dep in dependenciesSet)
            {
                deps[i++] = dep;
            }
            return deps;
        ...
        public static List<string> GetAllAssetBundles()
        {
            ...
            else
            {
                ...
                string[] bundles = _manifest == null ? _bundle2Dependencies.Keys.ToArray() : _manifest.GetAllAssetBundles();
                foreach (string bundle in bundles)
                {
                    assetBundleList.Add(bundle);
                }
