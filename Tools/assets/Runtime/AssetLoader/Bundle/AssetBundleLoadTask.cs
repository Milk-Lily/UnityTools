	...
	class AssetBundleLoadTask 
	{
        ...
        public void Init(string bundleName, DownloadService downloadService, BundleAssetLoadTask.LoadPriority priority, string assetPath, Action<string, string, int, float, string> observer)
		{
            ...
            else if (!isBundleExisted && downloadService != null) 
            {
                ...
                Debug.Log(string.Format("[ZeusSubpackage] Try to download and load assetBundle {0}.", abPath));
            }
            else
            {
                ...
                {
                    _createRequest = AssetBundle.LoadFromFileAsync(realPath, 0, offset);
                }
                else
                {
                    _createRequest = AssetBundle.LoadFromFileAsync(abPath);
                }
        ...
        private bool CheckSelfIsDone(bool isCheckAssetBundle = false)
        {
            ...
            if (!_bundleIsDone)
            {
                ...
                {
                    _createRequest = AssetBundle.LoadFromFileAsync(AssetBundleUtils.GetAssetBundlePath(BundleName));
                    _loadStep = LoadStep.LoadingBundle;
                    return false;
                }
        ...
        public bool IsRemoteLoading
        {
            get
            {
                ...
                for (int i = 0; i < _depLoadTask.Count; i++)
                {
                    if (_depLoadTask[i].IsRemoteLoading)
                    {
                        return true;
                    }
                }
