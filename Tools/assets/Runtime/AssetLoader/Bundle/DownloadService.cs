    ...
    public class DownloadService
    {
        ...
        public SubPackageBundleInfoContainer BundleInfoContainer
        {
            get
            {
                ...
                {
                    _bundleInfoContainer = SubPackageBundleInfoContainer.LoadSubPackageInfo();
                }
        ...
        bool _isSplitingThrowedError = false;
        ...
        private void SwitchStateToWaitLocalAreaNetwork()
        {
            ...
            OnDownloading?.Invoke(GetCompleteSize(), _totalSize, _avgSpeed, _state, SubpackageError.NetworkChange);
        }
        ...
        private void CheckError()
        {
            if (_subpackageError != SubpackageError.None)
            {
                ...
                {
                    Debug.LogError("SubpackageError is None.");
                }
            }
        }
        ...
        private void UpdateDownloadingInfo()
        {
            ...
            else
            {
                ...
                OnDownloading?.Invoke(GetCompleteSize(), _totalSize, _avgSpeed, SubpackageState.Downloading, SubpackageError.None);
        ...
        private void UpdateSplitingInfo()
        {
            if (Interlocked.Read(ref _totalSize) != -1)
            {
                ...
                OnDownloading?.Invoke(completedSize, totalSize, _avgSpeed, SubpackageState.Spliting, SubpackageError.None);
            }
        }
        ...
        void Finish(bool suc)
        {
            if (suc)
            {
                ...
                _tag2ChunkSet = null;
                OnDownloading?.Invoke(_totalSize, _totalSize, _avgSpeed, SubpackageState.Complete, SubpackageError.None);
                OnDownloading = null;
            ...
            else
            {
                if (_state == SubpackageState.SplitingError)
                {
                    long completedSize = Interlocked.Read(ref _splitingCompletedSize);
                    long totalSize = Interlocked.Read(ref _totalSize);
                    OnDownloading?.Invoke(completedSize, totalSize, _avgSpeed, SubpackageState.Abort, _subpackageError);
                }
                else
                {
                    if (!_isBackgroundDownloading)
                    {
                        OnDownloading?.Invoke(GetCompleteSize(), _totalSize, _avgSpeed, SubpackageState.Abort, _subpackageError);
                    }
                    else
                    {
            ...
            bool isDownloadAll)
        {
            ...
            switch (_state)
            {
                ...
                case SubpackageState.Abort:
                    ...
                    {
                        ThreadPool.QueueUserWorkItem(PrepareForDownloading, tags);
                    }
                    break;
                case SubpackageState.DownloadingError:
                case SubpackageState.SplitingError:
        ...
        private void Prepare(string[] tags, bool isDownloadAll)
        {
            _state = SubpackageState.Idle;
            _currentDownloadingTags = GetNewDownloadTags(tags, isDownloadAll);
            _isDownloadAll = isDownloadAll;
            ThreadPool.QueueUserWorkItem(PrepareForDownloading, _currentDownloadingTags);
        }
        ...
        private void PrepareForDownloading(System.Object o)
        {
#if DEVELOPMENT_BUILD
            string tagStr = string.Empty;
            if (_currentDownloadingTags != null && _currentDownloadingTags.Length > 0)
            {
                for (int i = 0; i < _currentDownloadingTags.Length; i++)
                {
                    tagStr += _currentDownloadingTags[i];
                    if (i < _currentDownloadingTags.Length - 1)
                    {
                        tagStr += ";";
                    }
                }
            ...
            lock (this)
            {
                try
                {
                    ...
                    else if (_state == SubpackageState.Abort || _state == SubpackageState.Pause || _state == SubpackageState.WaitLocalAreaNetwork)
                    {
                        InitVariable();
                        if (_isSplitingThrowedError)
                        {
                            ThreadPool.QueueUserWorkItem(UpdateSplitQueue);
                            _isSplitingThrowedError = false;
                        }
                        _state = SubpackageState.Ready;
                    }
                }
        ...
        private void SwitchStateToPause()
        {
            ...
            OnDownloading?.Invoke(GetCompleteSize(), _totalSize, _avgSpeed, _state, SubpackageError.Pause);
#if DEVELOPMENT_BUILD
            Debug.Log("SwitchStateToPause _state=" + _state);
#endif
        }
        ...
        Dictionary<string, string> _filePath2ChunkName = new Dictionary<string, string>();
        private void AddDownloadTask()
        {
            ...
            foreach (string chunkFile in _downloadSequence)
            {
                ...
                if (BundleInfoContainer.TryGetChunkInfo(chunkFile, out chunkInfo))
                {
                    ...
                    string outputPath = GetChunkPath(chunkFile);
                    //
                    _filePath2ChunkName.Add(outputPath, chunkFile);
        ...
        private void UpdateSplitQueue(System.Object o)
        {
            ...
            {
                string chunkName;
                while (_state != SubpackageState.Idle && _chunkSplitTaskQueue.TryDequeue(out chunkName))
                {
                    ...
                    if (BundleInfoContainer.TryGetChunkInfo(chunkName, out info))
                    {
                        ...
                        catch (Exception e)
                        {
                            ...
                            Debug.Log("Enqueue " + chunkName);
                            _isSplitingThrowedError = true;
                            if (e is IOException)
                            {
        ...
        {
            OnDownloading?.Invoke(_totalSize, _totalSize, _avgSpeed, _state, SubpackageError.None);
            _lastCompletedSizeQueue.Clear();
            for (int i = 0; i < SpeedCalcTimeSpan; ++i)
            {
