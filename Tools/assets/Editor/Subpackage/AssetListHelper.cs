...
namespace Zeus.Framework.Asset
{
    public static class AssetListHelper
    {
        ...
        public static List<string> LoadAssetListFromFiles(string[] tags, bool isParsing = true)
        {
            ...
            if (isParsing)
            {
                var excludeTags = TagAsset.LoadTagAsset().tagsAddToFirstPackage;
                ...
                foreach (var tag in tags)
                {
                    ...
                    {
                        addedTags.AddRange(excludeTags);
                        hasAdded = true;
                        break;
                    }
                }
            ...
            if (_files.Length > 0)
            {
                foreach (string tag in tags)
                {
                    if (tag == AssetBundleLoaderSetting.OthersTag)
                    {
                        ...
                        var recordedAssetList = LoadAssetListFromFiles(SubpackageWindow.TagAsset.TagListForRecord.ToArray());
