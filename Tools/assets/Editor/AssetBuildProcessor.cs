...
namespace Zeus.Framework.Asset
{
    public class AssetBuildProcessor : IModifyPlayerSettings, IBeforeBuild, IAfterBuild, IFinallyBuild
    {
        ...
        private static AssetManagerSetting _originSetting;
        ...
        private Dictionary<string, string> _bundle2ChunkName;
        private Dictionary<string, long> _bundle2ChunkFrom;
        private Dictionary<string, string> _other2ChunkName;
        ...
        {
            OverrideSettingByCommandLineArg();
        ...
        private void OverrideSettingByCommandLineArg()
        {
            _setting = AssetManagerSetting.LoadSetting();
            _originSetting = AssetManagerSetting.LoadSetting();
        ...
        private void ModifyBundleBuildPath(BuildTarget target)
        {
            if (_setting.assetLoaderType == AssetLoaderType.AssetBundle)
            {
                ...
                {
                    if (Path.GetExtension(filePath) == ".manifest" && !filePath.Contains(target + ".manifest"))
                    {
                        continue;
                    }
        ...
        private void CombineExcludeAssetBundle(string locationPathName, int chunkSizeInMB)
        {
            ...
            _bundle2ChunkName = new Dictionary<string, string>();
            _other2ChunkName = new Dictionary<string, string>();
            _chunkName2FinalName = new Dictionary<string, string>();
            _chunkName2Crc32 = new Dictionary<string, int>();
            _bundle2ChunkFrom = new Dictionary<string, long>();
            ...
            if (_setting.bundleLoaderSetting.mode == SubpackageMode.CreateOtherTagForUnrecordedAsset && _tag2AllFileSequence.Count > 0)
            {
        ...
        private List<string> DoCombine(List<AssetListHelper.FileSequence> files, string sourceDirectory, string locationPathName, long maxChunkSize, ref int totalChunkNum)
        {
            ...
            {
                if (files[i].isBundle) //bundle的处理
                {
                    var bundle = files[i];
                    string chunkName;
                    //检查是否这个Bundle已经被合并到某个Chunk了
                    if (_bundle2ChunkName.TryGetValue(bundle.fileName, out chunkName))
                    {
                ...
                {
                    string chunkName;
                    if (_other2ChunkName.TryGetValue(files[i].fileName, out chunkName))
                    {
                        if (chunkNameSet.Add(chunkName))
                        {
                            chunkNameList.Add(chunkName);
                    ...
                    if (chunkData.totalSize == 0)
                    {
                        chunkData.otherAssetList.Add(files[i].fileName);
                        chunkData.fileList.Add(files[i]);
                        chunkData = new ChunkData();
                        chunkList.Add(chunkData);
                    }
                    else
                    {
                        var aloneChunk = new ChunkData();
                        aloneChunk.otherAssetList.Add(files[i].fileName);
                        aloneChunk.fileList.Add(files[i]);
                        chunkList.Add(aloneChunk);
                    }
                }
            }
                ...
                if (chunkData.bundleList.Count + chunkData.otherAssetList.Count > 0)
                {
                    ...
                    using (FileStream fsWrite = File.Open(chunkPath, FileMode.OpenOrCreate, FileAccess.Write))
                    {
                        foreach (var bundleName in chunkData.bundleList)
                        {
                            ...
                            if (_setting.bundleLoaderSetting.useBundleScatter)
                            {
                                ckeckSumInfo.Update(CalcScatterBundlePath(sourceDirectory, bundleName), bundlePath, true);
                            }
                            else
                            {
                            ...
                            _bundle2ChunkFrom[bundleName] = bundlefrom;
                            bundlefrom += fileInfo.Length;
                            _bundle2ChunkName[bundleName] = chunkName;
                        }
                        foreach (var otherAssetPath in chunkData.otherAssetList)
                        {
                            var bundlePath = Path.Combine(Application.dataPath, otherAssetPath);

                            var relativePath = otherAssetPath.Substring(otherAssetPath.IndexOf("StreamingAssets") + 16);
                            ...
                            _other2ChunkName[otherAssetPath] = chunkName;
                        }
                    }
                }
            }
                ...
                if (!chunkSet.Add(name))
                {
                    ...
                    foreach (var chunkPair in _chunkName2FinalName)
                    {
                        if (name != chunkPair.Value) 
                            continue;
                        foreach (var pair in _other2ChunkName)
                        {
                        ...
                        {
                            foreach (var pair in _bundle2ChunkName)
                            {
                                if (chunkPair.Key != pair.Value) 
                                    continue;
                                conflictingFile1 = pair.Key;
        ...
        private void GenerateBundleInfoList(string chunkPath, Dictionary<string, long> chunkName2Size)
        {
            ...
            foreach (var pair in _tag2BundleSequence)
            {
                foreach (string bundle in pair.Value)
                {
                    ...
                    try
                    {
                        bundleInfos.Add(new SubPackageBundleInfo(bundle, crc32, (uint)_bundleName2Size[bundle], _chunkName2FinalName[_bundle2ChunkName[bundle]] + ChunkExtension, (uint)_bundle2ChunkFrom[bundle]));
                    }
                    catch
                    {
                        throw;
            ...
            foreach (var pair in _tag2OtherSequence)
            {
                foreach (string otherAsset in pair.Value)
                {
                    ...
                    otherAssetInfos.Add(new SubPackageOtherInfo(otherAssetRelativePath, _chunkName2FinalName[_other2ChunkName[otherAsset]] + ChunkExtension));
                }
            }
            //统计Chunk信息
        ...
        private void _RenameResources()
        {
            ...
            if (_setting.assetLoaderType == AssetLoaderType.AssetBundle)
            {
                if (AssetDatabase.IsValidFolder(resourcesRelativePath))
                {
                    ...
                    File.Move(resourcesRelativePath + ".meta", tempResourcesRelativePath + ".meta");
                }
                else
                {
        ...
        private void _RestoreResources()
        {
            ...
            if (_setting.assetLoaderType == AssetLoaderType.AssetBundle)
            {
                if (Directory.Exists(tempResourcesRelativePath))
                {
                    ...
                    File.Move(tempResourcesRelativePath + ".meta", resourcesRelativePath + ".meta");
                }
            }
        }
        ...
        {
            AssetManagerSetting.SaveSetting(_originSetting);
        }
